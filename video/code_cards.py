...
CARDS = [
    ...
    ("C09", "reclaim/brain/diagnosis/deterministic.py",
     "Layer 1 — free, instant, confidence 1.0",
     slice_("reclaim/brain/diagnosis/deterministic.py", 1, 12) + "\n\n" +
     slice_("reclaim/brain/diagnosis/deterministic.py", 18, 27), "python"),

    ("C10", "reclaim/brain/diagnosis/cohort.py",
     "Layer 1.5 — computed over the batch, before diagnosis",
     slice_("reclaim/brain/diagnosis/cohort.py", 1, 12) + "\n\n" +
     slice_("reclaim/brain/diagnosis/cohort.py", 17, 23), "python"),

    ("C11", "reclaim/brain/diagnosis/llm_diagnoser.py",
     "Layer 2 — the model, on the ambiguous rest only",
     slice_("reclaim/brain/diagnosis/llm_diagnoser.py", 1, 14) + "\n\n" +
     slice_("reclaim/brain/diagnosis/llm_diagnoser.py", 249, 258), "python"),

    ("C12", "reclaim/brain/policy/policies.yaml",
     "Rules are data — this row is REC_5042's silent retry",
     slice_("reclaim/brain/policy/policies.yaml", 1, 10) + "\n\n" +
     slice_("reclaim/brain/policy/policies.yaml", 12, 22), "yaml"),

    ("C13", "reclaim/brain/guardrails/base.py",
     "14 rules, one file each — and two non-negotiables",
     slice_("reclaim/brain/guardrails/base.py", 1, 17), "python"),

    ("C14", "reclaim/models.py  ·  reclaim/executor/actions.py",
     "Derived, never passed in — and claimed BEFORE the charge",
     slice_("reclaim/models.py", 84, 88) + "\n\n\n" +
     slice_("reclaim/executor/actions.py", 1, 15), "python"),
]
